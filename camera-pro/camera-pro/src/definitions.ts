...
export type CameraProPermissionState = PermissionState | 'limited';

export type CameraProPermissionType = 'camera' | 'photos';

export interface PermissionStatus {
  camera: CameraProPermissionState;
  photos: CameraProPermissionState;
}

export interface CameraProPluginPermissions {
  permissions: CameraProPermissionType[];
}
...
export interface CameraProPlugin {
   ...
   */
  requestPermissions(
    permissions?: CameraProPluginPermissions,
 ...
 * If the video was picked from the gallery, it will only be saved if edited.
 * @default: false
 *
 * @since 1.0.0
 */
...
export interface Video {
  ...
  webPath: string;
  /**
   * The format of the video, ex: mp4, 3gp.
   *
   * @since 1.0.0
   */
  format: string;
  /**
   * Whether if the video was saved to the gallery or not.
   *
   * On Android and iOS, saving to the gallery can fail if the user didn't
   * grant the required permissions.
   * On Web there is no gallery, so always returns false.
